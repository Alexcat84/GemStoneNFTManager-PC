class CodeGenerator {
     ...
     * @param {Array} gemstoneCodes - Array of gemstone codes
     * @param {number} month - Month (1-12)
     * @param {number} year - Year (e.g., 2025)
    ...
    /**
     * Generate QR code data for a full code
     * @param {string} fullCode - Full code
     * @param {Object} metadata - Additional metadata
     * @returns {string} QR code data
     */
    generateQRData(fullCode, metadata = {}) {
        const qrData = {
            code: fullCode,
            type: 'GemStone NFT',
    ...
    /**
     * Get next available piece number for given parameters
     * @param {Array} gemstoneCodes - Gemstone codes
     * @param {number} month - Month
     * @param {number} year - Year
     * @param {Function} getNextCorrelative - Database function to get next correlative
     * @returns {Promise<number>} Next piece number
     */
    async getNextPieceNumber(gemstoneCodes, month, year, getNextCorrelative) {
        try {
            return await getNextCorrelative(gemstoneCodes, month, year);
        } catch (error) {
            console.error('Error getting next piece number:', error);
     ...
     * @param {Function} getNextCorrelative - Database function
     * @returns {Promise<Array>} Array of generated codes
     */
    async generateMultipleCodes(gemstoneCodes, month, year, count, getNextCorrelative) {
        const codes = [];
        
        for (let i = 0; i < count; i++) {
            const pieceNumber = await this.getNextPieceNumber(gemstoneCodes, month, year, getNextCorrelative);
            const fullCode = this.generateCode(gemstoneCodes, month, year, pieceNumber);
            
            codes.push({
                fullCode,
                pieceNumber,
                gemstoneCodes: [...gemstoneCodes],
                month,
                year,
                checksum: this.generateChecksum(gemstoneCodes.join(','), month, year, pieceNumber)
            });
        }
    ...
    /**
     * Validate gemstone codes
     * @param {Array} gemstoneCodes - Array of gemstone codes to validate
     * @param {Array} validCodes - Array of valid gemstone codes from database
     * @returns {Object} Validation result
     */
    validateGemstoneCodes(gemstoneCodes, validCodes) {
        if (!gemstoneCodes || gemstoneCodes.length === 0) {
            return { valid: false, error: 'At least one gemstone must be selected' };
        }

        if (gemstoneCodes.length > 5) {
            return { valid: false, error: 'Maximum 5 gemstones allowed per code' };
        }

        const invalidCodes = gemstoneCodes.filter(code => !validCodes.includes(code));
        if (invalidCodes.length > 0) {
            return { valid: false, error: `Invalid gemstone codes: ${invalidCodes.join(', ')}` };
        }
    ...
    /**
     * Get gemstone display name from codes
     * @param {Array} gemstoneCodes - Array of gemstone codes
     * @param {Array} gemstones - Array of gemstone objects from database
     * @returns {string} Display name
     */
    getGemstoneDisplayName(gemstoneCodes, gemstones) {
        if (gemstoneCodes.length === 1) {
            const gemstone = gemstones.find(g => g.code === gemstoneCodes[0]);
            return gemstone ? gemstone.name : gemstoneCodes[0];
        } else {
            const names = gemstoneCodes.map(code => {
                const gemstone = gemstones.find(g => g.code === code);
