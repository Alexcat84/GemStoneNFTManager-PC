...
class PostgresDatabase {
    ...
    async getAllProducts() {
        try {
            ...
            console.log('🔍 [QR DATABASE] Raw products from DB:', result.rows);
            result.rows.forEach((product, index) => {
                console.log(`🔍 [QR DATABASE] Product ${index + 1}:`, {
                    id: product.id,
                    name: product.name,
        ...
        try {
            ...
            const result = await client.query(`
                UPDATE products SET
                    ...
                    is_featured = $17, is_archived = $18, updated_at = CURRENT_TIMESTAMP${soldDateClause}
                WHERE id = $19
                RETURNING *
            `, queryParams);
